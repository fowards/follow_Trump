    ...
    if data:
        ...
        for row in rows:
            ...
            if key not in index or len(tk) < len(index[key]):
                index[key] = tk
            _SEC_TITLE.setdefault(tk, title)
    _SEC_INDEX = index
    if not quiet:
...
def build_record(row, series):
    ...
    return {
        ...
        "companyEn": _SEC_TITLE.get(ticker) or TICKER_NAME_EN.get(ticker) or row["asset"][:60],
...
# OCR 잡음이 SEC 목록의 짧은 티커에 우연히 걸린 오탐. 검토로 확인해 제외한다.
#   PS  ← "powncemmomnsinc…" (순수 잡음)
#   EFOR← "EVERFORTH INC" (실존 대형주 아님, 훼손된 행)
DENY_TICKERS = {"PS", "EFOR"}
...
def build_from_ptrs(sources, enrich=True, use_ocr=True, dpi=300):
    ...
    for r in raw_rows:
        ...
        if not tk or tk in DENY_TICKERS:
            dropped += 1
            continue
