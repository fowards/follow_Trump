...
- **운영**: 로컬 PC의 `run_local.bat`이 매일 자동 갱신합니다(Windows 작업 스케줄러 등록 필요 — 아래 참조).
...
| 단계 | 하는 일 |
|------|---------|
| ① 자체 검증 | 파서 회귀 시험 60여 건. 실패하면 중단해 잘못된 데이터가 배포되지 않게 함 |
...
| ⑤ 시세 갱신 | Nasdaq(우선)·Stooq로 공시 후 추적 수익률 재계산 (Yahoo는 429 차단이 잦아 제외) |

### 회사명 → 티커 (SEC 무료 목록)

278-T에는 티커가 없고 회사명만 적혀 있습니다("ADOBE INC"). 수백 종목의 티커를
손으로 넣을 수 없어, SEC가 무료로 공개하는 공식 목록으로 해석합니다.

- 자동: 실행 시 `scripts/company_tickers.json`이 없으면 SEC에서 받아 캐시합니다.
- 수동(권장, 회사망에서 자동 다운로드가 막힐 때):
  <https://www.sec.gov/files/company_tickers.json> 를 받아 `scripts/company_tickers.json`으로 저장.

해석은 정규화 후 정확히 일치할 때만 티커를 부여하고, **못 찾으면 추측하지 않습니다**
(가짜 티커로 잘못된 데이터가 생기는 것을 막기 위함).
...
한 쪽만 골라 해상도(300/400/600) × 전처리(없음/선명화/2·3배확대+이진화) × psm(6/4)
조합을 돌려보고, **금액줄·거래행 수로 점수를 매겨** 최적 조합을 알려줍니다.
결과를 `run_local.bat` 상단의 `FT_OCR_DPI` / `FT_OCR_PSM` / `FT_OCR_PREP`에 적으면 됩니다.

> 실측(트럼프 2026 Q1·Q2 스캔본, 199dpi): **`psm=4` + `sharp`** 전처리가 결정적이었습니다.
> 기본값 `psm=6`으로는 금액 칸이 통째로 글자가 돼 한 건도 못 읽었지만(0행),
> `psm=4`(표를 단일 컬럼으로 읽음) + 대비 보정으로 한 페이지에서 금액 29줄이
> 살아났습니다. 그래서 기본값을 `psm=4` / `sharp`로 바꿔 두었습니다.

먼저 **원본 스캔의 실제 해상도**를 재서 알려줍니다. 이게 판단의 핵심입니다 —
PDF에 200dpi 이미지가 박혀 있으면 600dpi로 렌더링해도 없던 획이 생기지는 않습니다(보간일 뿐).
그런 문서는 해상도를 올려봐야 소용없고, 전처리에서 답이 나오거나 아니면 안 나옵니다.
...
무엇이 막혔는지 모를 때는 이걸 한 번 돌립니다. 모든 공시를 받아 파싱해 보고,
문서별로 읽은 방식(텍스트/OCR)·거래행·개별종목 수·원본 스캔 해상도를 표로 보여주며,
**걸러낸 행을 전부** 나열해 진짜 주식이 필터에 잘못 걸렸는지 눈으로 확인하게 해 줍니다.

```cmd
python scripts\build_data.py --diagnose
```

마지막에 판정을 냅니다 — 개별 종목이 0건일 때 그게 (a) OCR 화질 문제인지
(b) 필터가 과한지 (c) 실제로 채권뿐인지를 구분해 줍니다. PDF·OCR 캐시를 쓰므로
두 번째 실행부터는 빠릅니다.
